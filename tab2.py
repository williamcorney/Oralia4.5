...
class Tab2(QWidget):
    def __init__(self, *args, **kwargs):
        ...
        for i in range(4):
            ...
            button.setStyleSheet("""
                ...
                """)
            if i < 2: self.vertical_layout1.addWidget(button)
            else: self.vertical_layout2.addWidget(button)
    ...
    def generate_quiz(self, last_correct_answer=None):
        ...
        if self.mode == 'notes': query = f"SELECT note_file_name, note_display_name, note_clef, note_file_name2,difficulty FROM notes WHERE note_clef = '{self.clefftype}' AND difficulty = '{self.difficultylevel}'"
        else: query = f"SELECT signature_filename, signature_displayname FROM signatures WHERE signature_type = '{self.signaturetype}' AND difficulty = '{self.difficultylevel}'"
        rows = self.database_lookup(query)
        if last_correct_answer:rows = [row for row in rows if row[0] != last_correct_answer]
        try: rows
        ...
        try: wrong_rows = random.sample(remaining_rows, 3)
        except: print ('Error occurred at this place in code')
        try: wrong_rows
        except NameError: return
        wrong_answers = {
            "wrong_answer_1": {
    ...
    def load_quiz(self):
        ...
        if self.mode == 'notes': image_base_path = f"/Users/williamcorney/PycharmProjects/Oralia4.5/Images/Notes/{self.clefftype}/"
        else: image_base_path = "/Users/williamcorney/PycharmProjects/Oralia4.5/Images/Signatures/"
        ...
        for i, option in enumerate(options):
            ...
            try: self.option_buttons[i].clicked.disconnect()
            except TypeError:
                pass
